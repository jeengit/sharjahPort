			...
			oModel.read("/ManifestDetailsSet", {
				...
				success: function(data) {
					...
					if (status === 'CUST_APPROVED' || status === 'MAN_CREATED') {
						...
						for (var i in that.getView().byId("changeDetails").getItems()) {
							that.getView().byId("changeDetails").getItems()[i].setVisible(false);
						}
						that.getView().byId("changeDetails").getItems()[0].setVisible(true);
